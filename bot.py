...
COINGECKO_IDS = {
    "USDT": "tether", "BTC": "bitcoin", "TON": "the-open-network", "ETH": "ethereum", "SOL": "solana",
}
...
stats = {
    "unique_today": set(), "today_date": date.today(), "all_users": set(),
    "all_users_info": {}, "withdrawals": [],
}
...
def track_user(user):
    ...
    stats["all_users_info"][user.id] = {"username": user.username or "", "name": user.full_name or ""}
...
def track_withdrawal(user_id, username, crypto, amount, payout, currency, country, method, card="—"):
    ...
    stats["withdrawals"].append({
        "id": w_id, "user_id": user_id, "username": username or f"id{user_id}",
        "crypto": crypto, "amount": amount, "payout": payout, "currency": currency,
        "country": country, "method": method, "card": card,
        "time": datetime.now().strftime("%d.%m %H:%M"), "status": "pending",
    })
    return w_id
...
def _fetch_rates():
    ...
    resp = requests.get("https://api.coingecko.com/api/v3/simple/price",
        params={"ids": ids, "vs_currencies": currencies}, timeout=8).json()
    result = {}
    for cur in currencies.split(","):
        ...
        for symbol, cg_id in COINGECKO_IDS.items():
            try:
                ...
                result[cur][symbol] = round(FALLBACK_RUB[symbol] * FALLBACK_COEF.get(cur, 1.0), 6)
    return result

def get_rates(currency_code):
    cur = currency_code.lower()
    return _rates_cache.get(cur, {sym: round(FALLBACK_RUB[sym] * FALLBACK_COEF.get(cur, 1.0), 6) for sym in COINGECKO_IDS})
...
for country in COUNTRY_META.keys():
    countries_menu.add(country)
...
def rates_block(rates, currency):
    ...
    lines = [f"  {icons[sym]} {sym:<5} — {fmt(rates.get(sym, 0))} {currency}" for sym in ("USDT", "BTC", "TON", "ETH", "SOL")]
    return "\n".join(lines)
...
def withdrawal_admin_kb(w_id):
    ...
    kb.add(InlineKeyboardButton("✅ Принять", callback_data=f"approve:{w_id}"),
...
async def ask_for_card(chat_id, state, w_id):
    ...
    async with state.proxy() as data:
        data["w_id"] = w_id
    await bot.send_message(chat_id, f"💳 Введите номер карты\n\nПример: <code>1234 5678 9123 4567</code>\n\nТолько цифры:", parse_mode="HTML", reply_markup=types.ReplyKeyboardRemove())
...
async def exchange(message: types.Message):
    if not await check_ban(message):
        return
    track_user(message.from_user)
    await message.answer("🌍 Выберите страну выплаты\n\nКурс будет показан в валюте страны.", reply_markup=countries_menu)

@dp.message_handler(lambda m: m.text in COUNTRY_META, state="*")
async def country_selected(message: types.Message):
    if not await check_ban(message):
        return
    track_user(message.from_user)
...
async def crypto_selected(message: types.Message):
    if not await check_ban(message):
        return
    ...
    await message.answer(
        ...
        f"✍️ Введите количество {crypto}:", parse_mode="HTML")
...
async def card_entered(message: types.Message, state: FSMContext):
    ...
    if not card.isdigit() or not (13 <= len(card) <= 19):
        await message.answer("❌ Неверный формат.\n\nПример: <code>1234567891234567</code>", parse_mode="HTML")
        return
    ...
    if w:
        await bot.send_message(
            ADMIN_ID,
            f"🔔 <b>Заявка #{w['id']}</b>\n\n"
            f"👤 {uname}\n💸 {fmt(w['amount'], 6)} {w['crypto']} → ≈{fmt(w['payout'])} {w['currency']}\n"
...
async def cryptobot_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    currency = user_data[user_id]["currency"]
    payout = user_data[user_id]["payout"]
...
async def address_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"📥 Ручной перевод — {crypto}\n━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"{flag} {country}\n\n📬 Адрес:\n<code>{address}</code>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n💸 {fmt(amount, 6)} {crypto}\n💳 ≈{fmt(payout)} {currency}\n"
        f"📈 1 {crypto} = {fmt(price)} {currency}\n\n⏰ Переведите за 10 мин.", parse_mode="HTML")
    await callback.answer()
    await ask_for_card(callback.from_user.id, state, w_id)
...
async def approve_withdrawal(callback: types.CallbackQuery):
    ...
    w["status"] = "approved"
    try:
        await bot.send_message(w["user_id"], f"✅ <b>Заявка принята!</b>\n━━━━━━━━━━━━━━━━━━━━━━━\n💸 {fmt(w['amount'], 6)} {w['crypto']} → ≈{fmt(w['payout'])} {w['currency']}\n💳 <code>{w['card']}</code>\n🌍 {w['country']}\n\n⏱ Выплата 5–15 мин.\n👉 {SUPPORT}", parse_mode="HTML")
    except:
        pass
...
async def reject_withdrawal(callback: types.CallbackQuery):
    ...
    w["status"] = "rejected"
    try:
        await bot.send_message(w["user_id"], f"❌ <b>Заявка отклонена</b>\n━━━━━━━━━━━━━━━━━━━━━━━\n💸 {fmt(w['amount'], 6)} {w['crypto']} → ≈{fmt(w['payout'])} {w['currency']}\n🌍 {w['country']}\n\nОбратитесь в поддержку:\n👉 {SUPPORT}", parse_mode="HTML")
    except:
        pass
...
async def admin_enter(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        ...
        return
    await state.finish()
    await message.answer("🔐 <b>Панель администратора</b>\n━━━━━━━━━━━━━━━━━━━━━━━\nДобро пожаловать, Admin 👋", parse_mode="HTML", reply_markup=admin_menu)
...
async def cmd_stats(message: types.Message):
    ...
    await message.answer(f"📊 <b>Статистика</b>\n━━━━━━━━━━━━━━━━━━━━━━━\n📅 {today.strftime('%d.%m.%Y')}\n\n👥 Сегодня: <b>{len(stats['unique_today'])}</b>\n👤 Всего: <b>{len(stats['all_users'])}</b>\n\n📋 Заявок сегодня: <b>{len(w_today)}</b>\n📋 Всего заявок: <b>{len(stats['withdrawals'])}</b>\n⏳ Ожидают: <b>{len(pending)}</b>\n🚫 Заблокировано: <b>{len(blocked_users)}</b>", parse_mode="HTML")
...
async def cmd_users(message: types.Message):
    ...
    await message.answer(f"👥 <b>Пользователи</b>\n━━━━━━━━━━━━━━━━━━━━━━━\n\n{today_block}\n\nВсего: <b>{len(stats['all_users'])}</b>", parse_mode="HTML")
...
async def cmd_withdrawals(message: types.Message):
    ...
    if not withdrawals:
        await message.answer("📋 Заявок нет.", parse_mode="HTML")
        return
    last = withdrawals[-10:][::-1]
    await message.answer(f"📋 <b>Заявки</b> (последние {len(last)} из {len(withdrawals)})\n━━━━━━━━━━━━━━━━━━━━━━━", parse_mode="HTML")
    ...
    for w in last:
        ...
        text = f"<b>#{w['id']}</b> — {w['time']}\n👤 {uname}\n💸 {w['amount']} {w['crypto']} → ≈{fmt(w['payout'])} {w['currency']}\n💳 <code>{w['card']}</code>\n🌍 {w['country']} | {w['method']}\n{status_label.get(w['status'], '—')}"
...
@dp.message_handler(commands=["cancel"], state=[BlockState.waiting_for_user_id, BlockState.waiting_for_duration, UnblockState.waiting_for_user_id, BroadcastState.waiting_for_text, CardState.waiting_for_card])
...
async def block_get_user(message: types.Message, state: FSMContext):
    ...
    await message.answer("⏱ Срок:", reply_markup=block_duration_kb)
...
async def unblock_start(message: types.Message, state: FSMContext):
    if not blocked_users:
        await message.answer("ℹ️ Нет заблокированных.")
        return
    await UnblockState.waiting_for_user_id.set()
    lines = [f"@{stats['all_users_info'].get(uid, {}).get('username', f'id{uid}')}" for uid in blocked_users.keys()]
    await message.answer(f"✅ Разблокировка\n\n" + "\n".join(lines) + "\n\nВведите ID или @username:", reply_markup=types.ReplyKeyboardRemove())
...
async def broadcast_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"📣 Рассылка на {len(stats['all_users'])} чел.\n\nНапишите текст:", reply_markup=types.ReplyKeyboardRemove())
...
async def broadcast_send(message: types.Message, state: FSMContext):
    ...
    if not all_users:
        await message.answer("⚠️ Нет пользователей.", reply_markup=admin_menu)
        return
    sent = failed = 0
    ...
    await message.answer(f"✅ Отправлено: {sent}\n❌ Ошибка: {failed}", reply_markup=admin_menu)
...
async def amount_handler(message: types.Message):
    ...
    kb.add(InlineKeyboardButton("⚡ CryptoBot", callback_data="cryptobot"),
           InlineKeyboardButton("📥 На адрес", callback_data="address"))
    flag = COUNTRY_META[country][0]
    await message.answer(f"📋 Детали\n🔄 {fmt(amount, 6)} {crypto}\n💳 ≈{fmt(payout)} {currency}\n📈 1 {crypto} = {fmt(price)} {currency}\n{flag} {country}\n\nСпособ оплаты?", reply_markup=kb)
